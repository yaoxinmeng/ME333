...
display_msg = [newline "     a: Read current sensor (ADC counts)    b: Read current sensor (mA)" ...
               ...
               newline "     m: Load step trajectory                n: Load cubic trajectory" ...
               newline "     o: Execute trajectory                  p: Unpower the motor" ...
...
while ~has_quit
    ...
    switch selection
        ...
        case 'm'    % Load step trajectory
            ...
            ref = genRef(A, 'step');
            fprintf(mySerial,'%d\n',length(ref));   % write number of samples to serial port
            for i = 1:length(ref)
                fprintf(mySerial,'%d\n',ref(i));    % write to serial port
            end
